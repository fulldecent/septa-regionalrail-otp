...
$percentile = empty($_GET['percentile']) ? 5 : intval($_GET['percentile']);
$changeThresholdInMinutes = empty($_GET['threshold']) ? 1 : intval($_GET['threshold']);
?>
<!doctype html>
<html lang="en">
  <head>
...
foreach ($stops as $stop) {
  ...
  foreach ($trains as $train) {
    ...
    $pLate = percentile($latenessByDay, $percentile);
    $highlighted = $pLate >= $changeThresholdInMinutes;
    ...
    if ($highlighted) {
      $text = date("H:i",60*$pLate+strtotime($time));
