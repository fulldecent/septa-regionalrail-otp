...
<html lang="en">
  <head>
    <title><?= htmlentities($_GET['route']) ?> <?= $inbound ? 'Inbound' : 'Outbound' ?> Lateness</title>
  ...
  <body style="margin:1em">
    <h1><?= htmlentities($_GET['route']) ?> &mdash; Average Lateness (mins.)</h1>
  ...
  foreach ($trains as $train) {
    ...
    foreach ($stops as $stop) {
      if (empty($timetableByTrainAndStop[$train][$stop]) || empty($latenessByTrainDayAndTime[$train])) {
        ...
        $highlighted = average($dailyLatenesses) >= 3;
        ...
        $text  = sprintf("%.1f",average($dailyLatenesses));
